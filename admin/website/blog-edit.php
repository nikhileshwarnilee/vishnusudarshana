...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($slug !== '') {
        $dup = $pdo->prepare("SELECT id FROM blogs WHERE slug = ? AND id != ? LIMIT 1");
    ...
    if (empty($errors)) {
        ...
        $update = $pdo->prepare("UPDATE blogs SET title = ?, slug = ?, excerpt = ?, body = ?, tags = ?, cover_image = ?, video_url = ?, publish_date = ?, status = ? WHERE id = ?");
        $update->execute([
            $title,
            $slug,
            $excerpt,
    ...
    uploadBtn.addEventListener('click', function() {
        ...
        .then(data => {
            ...
            if (data.success) {
                ...
                if (previewDiv) {
                    const webPath = data.url || ('../../uploads/blogs/' + data.path);
