...
<head>
    ...
    <style>
        ...
        .back-link { color: #800000; font-weight: 600; text-decoration: none; display: inline-flex; align-items: center; gap: 8px; }
        .back-link:hover { text-decoration: underline; }
        .content-card { background: #fff; border-radius: 16px; box-shadow: 0 2px 16px rgba(224, 190, 190, 0.15); padding: 28px; margin-bottom: 24px; }
        .form-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(260px, 1fr)); gap: 18px 16px; }
        .form-group { display: flex; flex-direction: column; gap: 6px; }
        label { font-weight: 700; color: #800000; font-size: 0.95em; }
        input[type="text"], input[type="date"], input[type="url"], input[type="file"], select, textarea { padding: 12px 14px; border: 1px solid #e0bebe; border-radius: 10px; font-size: 1em; font-family: inherit; background: #fff; transition: border 0.2s ease, box-shadow 0.2s ease; }
        textarea { min-height: 140px; resize: vertical; }
        input[type="file"] { padding: 10px; background: #fff; }
        input:focus, select:focus, textarea:focus { outline: none; border-color: #800000; box-shadow: 0 0 0 3px rgba(128, 0, 0, 0.12); }
        .helper-text { color: #666; font-size: 0.9em; }
        .pill-note { display: inline-flex; align-items: center; gap: 8px; background: #f9eaea; color: #800000; padding: 10px 14px; border-radius: 12px; font-weight: 600; margin-bottom: 16px; border: 1px solid #f3caca; }
        .actions { display: flex; gap: 12px; flex-wrap: wrap; margin-top: 10px; }
        .btn-primary, .btn-secondary { border: none; border-radius: 10px; padding: 12px 20px; font-weight: 700; cursor: pointer; transition: all 0.2s ease; font-size: 1em; }
        .btn-primary { background: linear-gradient(135deg, #800000, #b36b00); color: #fff; box-shadow: 0 4px 12px rgba(128, 0, 0, 0.22); }
        .btn-primary:hover { transform: translateY(-2px); box-shadow: 0 6px 16px rgba(128, 0, 0, 0.28); }
        .btn-secondary { background: #f9eaea; color: #800000; border: 1px solid #f3caca; }
        .btn-secondary:hover { background: #f4dada; }
        .section-title { font-size: 1.1em; color: #800000; margin-bottom: 14px; font-weight: 700; }
        .two-col { display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 18px; }
        .editor-wrapper { border: 1px solid #e0bebe; border-radius: 12px; overflow: hidden; background: #fff; box-shadow: inset 0 1px 0 #f7f0f0; }
        .editor-toolbar { display: flex; flex-wrap: wrap; gap: 8px; padding: 10px 12px; background: #f9eaea; border-bottom: 1px solid #f3caca; }
        .editor-toolbar button { border: 1px solid #e0bebe; background: #fff; padding: 8px 10px; border-radius: 8px; cursor: pointer; font-weight: 700; color: #800000; transition: all 0.2s ease; }
        .editor-toolbar button:hover { background: #fdf7f7; box-shadow: 0 2px 8px rgba(128, 0, 0, 0.1); }
        .editor-area { min-height: 260px; padding: 14px; outline: none; font-size: 1em; line-height: 1.6; }
        @media (max-width: 768px) { .admin-container { padding: 22px 12px; } .content-card { padding: 20px; } h1 { font-size: 1.8em; } }
    </style>
</head>
<body>
...
<div class="admin-container">
    ...
    <div class="content-card">
        ...
        <?php endif; ?>
        <form id="blogEditForm" action="<?= htmlspecialchars($_SERVER['PHP_SELF']) . '?id=' . $editId ?>" method="post" enctype="multipart/form-data">
            ...
            <div class="actions">
                <button type="submit" class="btn-primary" id="saveBlogBtn">Update Blog</button>
