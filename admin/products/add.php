...
$product_name = $product_slug = $category_slug = $short_description = $price = $long_description = '';
$variant_id = '';
$is_active = 1;
$errors = [];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax'])) {
    ...
    if (!$errors) {
        ...
        $stmt = $pdo->prepare("INSERT INTO products (category_slug, product_name, product_slug, short_description, long_description, price, variant_id, is_active, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->execute([$category_slug, $product_name, $product_slug, $short_description, $long_description, $price, $variantIdForSave, $is_active, $createdAt]);
        ...
        $.ajax({
            ...
            success: function(res) {
                if (res.success) {
                    ...
                    if (window.tinymce && window.tinymce.get('long_description_editor')) {
                        window.tinymce.get('long_description_editor').setContent('');
                    }
