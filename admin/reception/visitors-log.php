    ...
    .then(data => {
        ...
        if (data.success && data.data.length) {
            data.data.forEach(row => {
                let statusOptions = ['open','closed','cancelled'];
                let statusColors = {open:'#0056b3',closed:'#1a8917',cancelled:'#a00'};
                let statusDropdown = `<select class="status-dropdown" data-id="${row.id}" style="background:#f9eaea;color:${statusColors[row.status]};font-weight:600;border-radius:7px;padding:5px 12px;border:1px solid #e0bebe;outline:none;">
