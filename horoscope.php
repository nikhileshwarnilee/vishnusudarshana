...
try {
    foreach ($languages as $langCode) {
        foreach ($zodiacs as $zodiacNum => $zodiacName) {
            ...
            $stmt->execute([$zodiacNum, $langCode]);
            $row = $stmt->fetch();
            if ($row && $row['horoscope_json']) {
                $horoscopeByZodiacLang[$langCode][$zodiacNum] = json_decode($row['horoscope_json'], true);
            }
        }
    }
} catch (Exception $e) {
...
// Zodiac labels for different languages
$zodiacLabels = [
    'en' => $zodiacs,
    'hi' => ['मेष', 'वृष', 'मिथुन', 'कर्क', 'सिंह', 'कन्या', 'तुला', 'वृश्चिक', 'धनु', 'मकर', 'कुंभ', 'मीन'],
    'mr' => ['मेष', 'वृष', 'मिथुन', 'कर्क', 'सिंह', 'कन्या', 'तुला', 'वृश्चिक', 'धनु', 'मकर', 'कुंभ', 'मीन'],
    'gu' => ['મેષ', 'વૃષ', 'મિથુન', 'કર્ક', 'સિંહ', 'કન્યા', 'તુલા', 'વૃશ્ચિક', 'ધનુ', 'મકર', 'કુંભ', 'મીન'],
    'ka' => ['ಮೇಷ', 'ವೃಷ', 'ಮಿಥುನ', 'ಕರ್ಕ', 'ಸಿಂಹ', 'ಕನ್ಯಾ', 'ತುಲಾ', 'ವೃಶ್ಚಿಕ', 'ಧನು', 'ಮಕರ', 'ಕುಂಭ', 'ಮೀನ'],
    'te' => ['మేషం', 'వృషభం', 'మిథునం', 'కర్కటకం', 'సింహం', 'కన్యా', 'తుల', 'వృశ్చికం', 'ధనుస్సు', 'మకరం', 'కుంభం', 'మీనం']
];
?>
